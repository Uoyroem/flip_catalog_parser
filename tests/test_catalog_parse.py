...
def test_parse_and_get_products():
    post_url = f"{BASE_URL}/catalogs/parse-products/by-url"
    print(f"\nОтправка POST-запроса на: {post_url}")

    try:
        post_response = requests.post(
            post_url, params={"url": "https://www.flip.kz/catalog?subsection=2649"}
        )
    ...
    get_url = f"{BASE_URL}/products"
    print(f"\nОтправка GET-запроса на: {get_url}")

    try:
        get_response = requests.get(get_url, params={"limit": 10})
